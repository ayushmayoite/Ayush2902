...
const AFC_DIR = path.resolve(ROOT, "public", "images", "afc");
const OUT_MANIFEST = path.resolve(ROOT, "scripts", "afc-image-manifest.csv");
...
async function main(): Promise<void> {
  if (!(await fs.pathExists(AFC_DIR))) {
    throw new Error(`AFC image directory not found: ${AFC_DIR}`);
  }

  const dirEntries = await fs.readdir(AFC_DIR, { withFileTypes: true });
  ...
  for (const slug of productDirs) {
    ...
    const folderPath = path.join(AFC_DIR, slug);
