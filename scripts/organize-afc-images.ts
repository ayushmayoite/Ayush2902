...
const RAW_DIR = path.resolve(ROOT, "scripts", "afc-raw");
const MANIFEST_PATH = path.resolve(ROOT, "scripts", "afc-image-manifest.csv");
const REPORT_PATH = path.resolve(ROOT, "scripts", "afc-image-report.json");
const POST_SYNC_AUDIT_PATH = path.resolve(
  ROOT,
  "scripts",
  "afc-postsync-audit.json",
);
  ...
  for (const [slug, slugRows] of grouped) {
    ...
    for (const rank of ranks) {
      ...
      if (!sourceAbsolutePath.startsWith(RAW_DIR)) {
        throw new Error(
          `Slug ${slug} has invalid source path outside afc-raw: ${row.source_relative_path}`,
        );
      }
